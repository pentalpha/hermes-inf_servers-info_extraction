...
class TritonPythonModel:
    ...
    def classify_by_schema(self, classification_schema, transcript):
    ...
    def single_inference(self, transcript_full, classification_schema_str):

        classification_schema_dict = json.loads(classification_schema_str)
        transcript_parts = sliding_window_over_paragraph(transcript_full, 
                                                        n_words=MAX_TOKENS, 
                                                        sobreposicao=5)
        to_join = []
        gpu_usage_secs = 0.0
            ...
            if "boolean" in classification_schema_dict:
                ...
                for key, value in classification_schema_raw.items():
                    ...
                    for entity_name, info in value.items():
                        ...
                        nome_sim = info["sim"]
                        nome_nao = info["nao"]
                        entity_descs.append(f"{entity_name}::[{nome_sim}|{nome_nao}]::str::{desc}")
                        entity_names.append(entity_name)
                    classification_schema_1[key] = entity_descs
                    multilabel_schemas[key] = entity_names

                #print(classification_schema_1, file=sys.stderr)
                new_entities, req_duration2 = self.classify_by_schema(classification_schema_1, transcript)
                gpu_usage_secs += req_duration2
                #new_entities = {}
                #req_duration2 = 1
                #print('new_entities:', new_entities, file=sys.stderr)
                results_multilabel0 = []

                for label_group in new_entities.keys():
                    for nat in new_entities[label_group]:
                        for label_name, value in nat.items():
                            if value is not None:
                                info = classification_schema_raw[label_group][label_name]
                                text = value['text']
                                conf = value['confidence']

                                if text == info['sim']:
                                    results_multilabel0.append({"label": label_name, "confidence": conf})
                                elif text == info['nao']:
                                    results_multilabel0.append({"label": label_name, "confidence": 1-conf})
                #print('results_multilabel0:', results_multilabel0, file=sys.stderr)
                for ml_schema_name, ml_categories in multilabel_schemas.items():
                    ml_schema = self.model.create_schema().classification(
                        ml_schema_name,
                        ml_categories,
                        multi_label=True,
                        cls_threshold=0.01  # Lower threshold for multi-label
                    )
                    #print(ml_schema, file=sys.stderr)
                    extract_start = time.time()
                    results_multilabel = self.model.extract(transcript, ml_schema, include_confidence=True)
                    extract_finish = time.time()
                    req_duration3 = extract_finish - extract_start
                    gpu_usage_secs += req_duration3
                    #print('results_multilabel: ', json.dumps(results_multilabel, ensure_ascii=False, indent=3), file=sys.stderr)
                    categories_found = [label_info["label"] for label_info in results_multilabel[ml_schema_name]]
                    not_found = [label for label in ml_categories if label not in categories_found]
                    for label in not_found:
                        results_multilabel0.append({"label": label, "confidence": 0.0})
                    results_multilabel0 += results_multilabel[ml_schema_name]
        ...
        if len(to_join) == 0:
            return {}, gpu_usage_secs
        else:
            final_entities = join_entity_predictions_mean(to_join)
            return final_entities, gpu_usage_secs
